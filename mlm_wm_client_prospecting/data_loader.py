...
CATEGORICAL_COLUMNS = [data_mappings[col] for col in CATEGORICAL_COLUMNS]
STRING_COLUMNS = [data_mappings[col] for col in STRING_COLUMNS]
class DataLoader(object):
    def __init__(self, df=None):
        self.data = df
        self.data = self.data[DD2_COLUMNS]
        self.data.rename(columns=data_mappings, inplace=True)
